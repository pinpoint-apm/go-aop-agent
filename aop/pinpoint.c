...
int32_t detour_does_code_end_function_e9(OpcodeType pbCode)
{
    if (OPCODE_1(pbCode) == 0xeb || // jmp +imm8
        ...
        OPCODE_1(pbCode) == 0xcc)
    { // brk
        return 1;
    }
    else if (OPCODE_1(pbCode) == 0xf3 && OPCODE_2(pbCode) == 0xc3)
    { // rep ret
        return 1;
    }
    else if (OPCODE_1(pbCode) == 0xff && OPCODE_2(pbCode) == 0x25)
    { // jmp [+imm32]
        return 1;
    }
...
int32_t detour_does_code_end_function(OpcodeType pbCode)
{
    ...
    else if (OPCODE_1(pbCode) == 0xf3 && OPCODE_1(pbCode) == 0xc3)
    { // rep ret
        return 1;
    }
    else if (OPCODE_1(pbCode) == 0xff && OPCODE_2(pbCode) == 0x25)
